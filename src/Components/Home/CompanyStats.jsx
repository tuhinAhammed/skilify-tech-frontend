...
const CompanyStats = () => {
    ...
    return (
        ...
        >
            ...
            <Container>
                <div className="relative px-12 py-sectionLg rounded-[2vw]"
                    style={{ backgroundImage: `url(${statsBg})` }}
                >
                    <div className="absolute inset-0 bg-black/80 pointer-events-none rounded-[2vw]"></div>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 justify-between z-10">
    {/* Left Section */}
    <div>
    ...
    <div className="pt-6 md:pt-10 grid grid-cols-1 gap-y-6 z-10 w-full">
        {statsData.map((item, index) => (
            <div
                key={index}
                className="flex flex-wrap md:flex-nowrap justify-between items-center p-2 group transition-all bg-theme hover:bg-secondary rounded-full duration-500"
            >
                ...
                <MidTitle
                    className="text-primary md:pr-12 !uppercase pt-2 md:pt-0"
                    text={item.title}
                />
            </div>
        ))}
